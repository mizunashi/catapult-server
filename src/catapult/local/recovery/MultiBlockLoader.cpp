	...
	public:
		model::ChainScore loadAll(const NotifyProgressFunc& notifyProgress) const {
			...
			while (chainHeight >= height) {
				...
				auto statusConsumer = m_statusConsumer;
				stateHash = execute(*pBlockElement, [&previousScore, &score, &blockElement, statusConsumer](auto&& cacheChanges) {
					auto scoreDelta = previousScore - score;
