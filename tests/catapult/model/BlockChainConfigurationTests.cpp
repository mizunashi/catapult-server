	...
	namespace {
		...
		struct BlockChainConfigurationTraits {
			...
			static utils::ConfigurationBag::ValuesContainer CreateProperties() {
				return {
					...
					},
					{
						"fork_heights",
						{
							{ "calculateTransactionFeeOverflowFix", "3333" }
						}
					},
					{
						"plugin:alpha",
			...
			static void AssertZero(const BlockChainConfiguration& config) {
				...
				EXPECT_EQ(Height(), config.ForkHeights.CalculateTransactionFeeOverflowFix);
			...
			static void AssertCustom(const BlockChainConfiguration& config) {
				...
				EXPECT_EQ(Height(3333), config.ForkHeights.CalculateTransactionFeeOverflowFix);
